...
if ($pingTest.token_valid)
{
	...
	### Check if TV matches local device information
	if (($TVDeviceInfo.alias -eq $DeviceAlias) -and ($TVDeviceInfo.groupid -eq $DeviceGroup))
	{
		return 'Compliant'
	}
	else
